...
const Card = ({ rating, title, imageUrl, level, description, instructorImageUrl, instructorName, price }) => {
  return (
    <div className="max-w-sm rounded overflow-hidden shadow-lg bg-white p-4 text-justify">
      <div className="bg-purple-900 rounded-t-lg p-4 flex justify-between items-center">
      ...
      <div className="px-6 pt-4 pb-2 flex justify-between items-center">
        <div className="flex items-center">
          ...
          <span className="ml-2 text-purple-700 font-semibold">{instructorName}</span>
        </div>
        <span className="text-purple-500 text-xl font-semibold">${price}</span>
      </div>
    </div>
  );
};
